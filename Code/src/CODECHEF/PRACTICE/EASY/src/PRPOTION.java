...
/**
 * Created by Shreyans Sheth [bholagabbar] on 7/2/2015 at 4:36 PM using IntelliJ IDEA (Fast IO Template)
 */
...
class PRPOTION
{
    public static void main(String[] args) throws Exception
    {
        ...
        while(t-->0)
        {
            ...
            for(int i=0;i<n1;i++)
            {
                max1=Math.max(in.readInt(),max1);
            }
            for(int i=0;i<n2;i++)
            {
                max2=Math.max(in.readInt(),max2);
            }
            for(int i=0;i<n3;i++)
            {
                max3=Math.max(in.readInt(),max3);
            }
