...
class BreakStringIntoWords {
    ...
    private static boolean isWord(String s) {
        if (s.equals("take") || s.equals("bath") || s.equals("bat") || 
                s.equals("hand") || s.equals("and") || s.equals("come")) {
            return true;
        }
        return false;
    }
    
    public static void rec(String ori, String s, int cnt) {
        if (cnt == ori.length()) {
            ans++;
            return;
        }
        for (int i = 1; i <= s.length(); i++) {
            ...
            if (isWord(currString)) {
                rec(ori, s.substring(i, s.length()), cnt + currString.length());
            }
        }
    }
