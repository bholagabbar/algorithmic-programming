    ...
    public static ArrayList<Integer> prevSmaller(ArrayList<Integer> arr) {
        ...
        for (int i = arr.size() - 1; i >= 0; i--) {
            if (!st.isEmpty()) {
                int curr = st.pop();
                if (curr >= arr.get(i)) {
                    Stack<Integer> putAgain = new Stack<>();
                    while (!st.isEmpty() && st.peek() >= arr.get(i)) {
                        putAgain.push(st.pop());
                    }
                    if (!st.isEmpty()) {
                        curr = st.pop();
                        ans.set(i, curr);
                    ...
                    while (!putAgain.isEmpty()) {
                        st.push(putAgain.pop());
                    }
                } else {
                    ans.set(i ,curr);
                }
...
//        System.out.println(ans);
    ...
    public static void main(String[] args) throws Exception {
        ...
        prevSmaller(new ArrayList<>(Arrays.asList(34, 35, 27, 42, 5, 28, 39, 20, 28 )));
    }
