...
class StringAllPalindromes {
    ...
    private static boolean isPalindrome(int start, int end) {
        if (ori.substring(start, end).hashCode() == (new StringBuilder(ori.substring(start, end)).reverse().toString().hashCode()))
            return true;
        return false;
    }
    
    private static void recurse(int start, ArrayList<String> arr) {
        if (N == start) {
        ...
        for (int i = start; i < N; i++) {
            if (isPalindrome(start, i + 1))  {
                arr.add(ori.substring(start, i + 1));
                recurse(i + 1, arr);
