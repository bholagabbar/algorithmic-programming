...
class AssignMiceToHoles {
    ...
    public static int mice(ArrayList<Integer> a, ArrayList<Integer> b) {
        ...
        for (int i = 0; i < a.size(); i++) {
            Integer x = ts.ceiling(a.get(i));
            Integer y = ts.floor(a.get(i));
            Integer remove = null;
            int min = Integer.MAX_VALUE;
            if (x != null) {
                min = Math.min(min, Math.abs(x - a.get(i)));
                remove = x;
            }
            if (y != null) {
                min = Math.min(min, Math.abs(y - a.get(i)));
                remove = y;
            }
            fans = Math.max(fans, min);
            if (remove != null) {
                ts.remove(remove);
            }
        }
    ...
    public static void main(String[] args) throws Exception {
        ...
        mice(new ArrayList<>(Arrays.asList(4, -4, 2)), new ArrayList<>(Arrays.asList(4, 0, 5)));
    }
