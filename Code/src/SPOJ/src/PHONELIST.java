...
class PHONELIST {
    ...
    static TrieNode trieHead;

    static void insertIntoTrie(String toInsert) {
        TrieNode currNode = trieHead;
        for (int i = 0; i < toInsert.length(); i++) {
            char currChar = toInsert.charAt(i);
            int valToInsert = currChar - 48;
    ...
    static boolean queryTrie(String[] list) {
        Arrays.sort(list);
        HashSet<String> storePref = new HashSet<>();
        for (String toFind: list) {
            StringBuilder prefix = new StringBuilder();
            TrieNode currNode = trieHead;
            int i = 0;
            while (i < toFind.length() && currNode.next[toFind.charAt(i) - 48].cnt > 1) {
                prefix.append(Character.toString(toFind.charAt(i)));
                if (storePref.contains(prefix.toString())) {
                    return false;
                }
                currNode = currNode.next[toFind.charAt(i) - 48];
                i++;
            }
            if (i < toFind.length())  {
                prefix.append(Character.toString(toFind.charAt(i)));
            }
            String fPref = prefix.toString();
            if (!storePref.contains(fPref)) {
                storePref.add(fPref);
            } else {
               return false;
            }
